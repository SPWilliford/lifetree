...
// One prepared statement, finalized on scope exit. Parameters bind in
// CALL ORDER, not by index.
class Statement {
public:
    ...
    // Smaller integer types promote to int; time_t lands on the 64-bit one.
    ...
    Statement& bind(std::string_view value) {
        // TRANSIENT: sqlite copies. Length passed because a string_view may
...
Database::Database(const std::string& path) {
    ...
    // Before create_schema(), which would make every database look existing.
    const bool fresh = !has_table("life_tree");

    // TRAP: outside any transaction — PRAGMA foreign_keys is a silent no-op
    // inside one. The rebuild in migrate() drops both tree tables and a
    // cascade would take every weight, link and color with them.
    execute("PRAGMA foreign_keys = OFF;");

    // Ordered: a fresh database must not run ALTER TABLE on tables that
    // don't exist yet.
    create_schema();
    if (fresh)
...
void Database::migrate() {
    ...
    // v3 has no work left to do; the stamp stays so a v2 database advances.
    if (schema_version() < 3) {
...
void Database::clear_spawned_instances() {
    // Recursive, not a single DELETE on parent_id: foreign keys are OFF
    // throughout migrate(), so ON DELETE CASCADE will not fire.
    execute("BEGIN;");
...
void Database::rebuild_trees_with_autoincrement() {
    ...
    // itself. TRAP: ids are copied explicitly, which seeds sqlite_sequence;
    // otherwise a deleted id gets reused and the next node inherits its
    // color, schedule and links from the caches still holding them.
    //
...
bool Database::has_column(const std::string& table, const std::string& column) {
    // PRAGMA won't take a bound parameter; every caller passes a literal.
...
std::vector<WorkLogRow> Database::load_work_log_for_day(time_t day) {
    ...
    const time_t day_start = clock_util::local_midnight(day);
    const time_t day_end = day_start + 24 * 60 * 60;
...
std::vector<CompletedTaskSummary> Database::load_completed_tasks_for_day(time_t day) {
    ...
    Statement stmt(m_db,
                   ...
                   "FROM work_log "
                   // completed_at = 0 falls outside any day's range.
...
long Database::open_work_seconds(int source_task_id) {
    // COALESCE: SUM over no rows is NULL.
    Statement stmt(m_db,
